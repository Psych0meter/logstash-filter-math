...
Each inner array is either a *binary* calculation with 4 elements
(`[operator, left_operand, right_operand, target]`) or a *unary* calculation
with 3 elements (`[operator, operand, target]`, used for `abs`, `neg`, `round`-free
single-operand operators, and the unit conversion operators below).
...
| Abs (unary) | 'abs'
| Negate (unary) | 'neg', 'negate'
| Floor (unary) | 'floor'
| Ceiling (unary) | 'ceil'
| Sign (unary) | 'sign'
| Square root (unary) | 'sqrt'
| Cube root (unary) | 'cbrt'
| Natural logarithm (unary) | 'ln'
| Base-10 logarithm (unary) | 'log10'
| Minimum | 'min'
| Maximum | 'max'
| Percent change | 'percent_change'
| Miles to kilometers (unary) | 'mi_to_km'
| Kilometers to miles (unary) | 'km_to_mi'
| Meters to feet (unary) | 'm_to_ft'
| Feet to meters (unary) | 'ft_to_m'
| Celsius to Fahrenheit (unary) | 'c_to_f'
| Fahrenheit to Celsius (unary) | 'f_to_c'
| Bytes to kilobytes (unary, 1 KB = 1024 bytes) | 'bytes_to_kb'
| Kilobytes to bytes (unary) | 'kb_to_bytes'
| Bytes to megabytes (unary) | 'bytes_to_mb'
| Megabytes to bytes (unary) | 'mb_to_bytes'
| Bytes to gigabytes (unary) | 'bytes_to_gb'
| Gigabytes to bytes (unary) | 'gb_to_bytes'
| Milliseconds to seconds (unary) | 'ms_to_s'
| Seconds to milliseconds (unary) | 's_to_ms'
|=======================================================================
...
For a *binary* calculation (4 elements):
...
For a *unary* calculation (3 elements, e.g. `abs`, `neg`, or one of the unit conversion
operators), the second element is the single operand (field, literal, or memory register)
and the third element is the "target", with the same rules as above.
...
===== `tag_on_failure`
  ...
  * Default value is `["_mathexception"]`

If a calculation raises an unexpected runtime error (for example, an extremely large
or otherwise out-of-range operand to `round`), that single calculation is skipped, the
event is tagged with the values in this setting, and the remaining calculations in
`calculate` still run. This prevents one malformed event from crashing the pipeline.
...
The `Sqrt` operator has a check for a negative operand, and `Ln`/`Log10` have a check for a
zero or negative operand, for the same reason (the real result is undefined or complex). +
`Cbrt` has no such restriction since the real cube root is defined for negative numbers. +
...
[NOTE]
As with the `Power`/`Divide` literal checks, if you pass a literal operand to `sqrt`, `ln`,
or `log10` that falls outside the valid domain (e.g. `[ "sqrt", -4, "result" ]`), this is
detected at plugin registration time and raises a configuration error rather than silently
skipping the calculation on every event.
