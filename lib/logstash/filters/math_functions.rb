  ...
  module Filters
    module MathFunctions
      ...
      class Sign
        ...
        def name; "sign"; end
        def call(op1); op1 <=> 0; end
      end

      # Shared validity check for functions that are undefined for negative operands
      # (square root and logarithms of a negative real number are complex, which cannot
      # be stored in an event or serialized to JSON).
      module NonNegativeValidityCheck
        def invalid?(op1, op2 = nil, event = nil)
          if op1.is_a?(Numeric) && op1 < 0
            warning = "#{name} of a negative number results in a complex number that cannot be stored in an event"
            if event
              logger.warn(warning, "operand 1" => op1, "event" => event.to_hash)
              return true
            else
              return warning
            end
      ...
      class Sqrt
        include UnaryFunction
        include LogStash::Util::Loggable
        include NonNegativeValidityCheck

        def name; "sqrt"; end
        def call(op1); Math.sqrt(op1); end
      end
      ...
      class Cbrt
        ...
        def name; "cbrt"; end
        def call(op1); Math.cbrt(op1); end
      end

      # Shared validity check for logarithms: undefined (complex) for negative operands,
      # and negative infinity for zero, which also can't be stored in an event.
      module PositiveValidityCheck
        def invalid?(op1, op2 = nil, event = nil)
          if op1.is_a?(Numeric) && op1 <= 0
            warning = "#{name} of a non-positive number is undefined or results in a value that cannot be stored in an event"
            if event
              logger.warn(warning, "operand 1" => op1, "event" => event.to_hash)
              return true
            else
      ...
      class Ln
        include UnaryFunction
        include LogStash::Util::Loggable
        include PositiveValidityCheck

        def name; "ln"; end
        def call(op1); Math.log(op1); end
      end
      ...
      class Log10
        ...
        def name; "log10"; end
        def call(op1); Math.log10(op1); end
      end
      ...
      class Min
        ...
        def name; "min"; end
        def call(op1, op2); [op1, op2].min; end
      end
      ...
      class Max
        ...
        def name; "max"; end
        def call(op1, op2); [op1, op2].max; end
      end
      ...
      def self.default_conversions
        {
          ...
          # data size (binary/IEC units, i.e. 1 KB = 1024 bytes)
          "bytes_to_kb" => Conversion.new("bytes_to_kb", 1.0 / 1024.0, 0.0),
          "kb_to_bytes" => Conversion.new("kb_to_bytes", 1024.0, 0.0),
          "bytes_to_mb" => Conversion.new("bytes_to_mb", 1.0 / (1024.0 ** 2), 0.0),
          "mb_to_bytes" => Conversion.new("mb_to_bytes", 1024.0 ** 2, 0.0),
          "bytes_to_gb" => Conversion.new("bytes_to_gb", 1.0 / (1024.0 ** 3), 0.0),
          "gb_to_bytes" => Conversion.new("gb_to_bytes", 1024.0 ** 3, 0.0),
          # time
          "ms_to_s" => Conversion.new("ms_to_s", 1.0 / 1000.0, 0.0),
          "s_to_ms" => Conversion.new("s_to_ms", 1000.0, 0.0)
        }
      end
    end
  end
